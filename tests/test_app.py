...
def test_get_states_no_cache(mock_get_all_states, mock_redis_get, client):
...
@patch('app.redis_client.get')
def test_get_states_with_cache(mock_redis_get, client):
...
def test_get_districts_missing_param(client):
    response = client.get("/districts")
    assert response.status_code == 422

@patch('app.redis_client.get')
@patch('app.get_districts_by_state')
def test_get_districts_no_cache(mock_get_districts, mock_redis_get, client):
    mock_redis_get.return_value = None
...
@patch('app.redis_client.get')
@patch('app.get_districts_by_state')
def test_get_districts_not_found(mock_get_districts, mock_redis_get, client):
    mock_redis_get.return_value = None
...
def test_get_centers_missing_param(client):
...
def test_get_centers_no_cache(mock_get_centers, mock_redis_get, client):
    mock_redis_get.return_value = None
...
def test_clear_cache(mock_delete, mock_keys, client):
